...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"0 0 0px","margin":"0px 0 0","borderColor":"#ddd","alignItems":"center","display":"flex","clear":"both","flexWrap":"wrap","background":"#fff","borderWidth":"0 0 0px","width":"100%","borderStyle":"solid","height":"auto","order":"1"}'>
      <el-form-item :style='{"margin":"0 10px 0 0"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 0 0 10px","lineHeight":"42px","textAlign":"right","display":"inline-block"}'>诊疗项目：</div>
        <el-input v-model="formSearch.zhenliaoxiangmu" placeholder="诊疗项目" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"0 10px 0 0"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 0 0 10px","lineHeight":"42px","textAlign":"right","display":"inline-block"}'>医院名称：</div>
        <el-input v-model="formSearch.yiyuanmingcheng" placeholder="医院名称" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"0 10px 0 0"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 0 0 10px","lineHeight":"42px","textAlign":"right","display":"inline-block"}'>医院地址：</div>
        <el-input v-model="formSearch.yiyuandizhi" placeholder="医院地址" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
	  ...
	  <el-button v-if="btnAuth('chongwuyiliao','新增')" :style='{"cursor":"pointer","border":"0px solid #ddd","padding":"0px 10px","margin":"0px 10px 0 0","color":"#fff","borderRadius":"0px","background":"rgba(30,124,9,1)","display":"inline-block","width":"80px","fontSize":"14px","lineHeight":"36px","height":"36px"}' type="primary" @click="add('/index/chongwuyiliaoAdd')"><i v-if="true" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
		  ...
		  <div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
		    ...
		    <div :style='{"width":"100%","padding":"0 0px","overflow":"hidden","alignItems":"center","display":"flex","height":"auto"}' class="item-info">
		      <div :style='{"width":"100%","padding":"0 0px 0 0","textAlign":"center","flex":"1"}'>
				<div class="title">{{item.zhenliaoxiangmu}}</div>
				<div class="title">{{item.yiyuanmingcheng}}</div>
				<div class="title">{{item.yiyuandizhi}}</div>
		      ...
		      <div class="desc" v-html="item.yiyuanjianjie"></div>
		    </div>
		  </div>
		</div>
	</div>
  ...
  export default {
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        let searchWhere = {};
        if (this.formSearch.zhenliaoxiangmu != '') searchWhere.zhenliaoxiangmu = '%' + this.formSearch.zhenliaoxiangmu + '%';
        if (this.formSearch.yiyuanmingcheng != '') searchWhere.yiyuanmingcheng = '%' + this.formSearch.yiyuanmingcheng + '%';
        if (this.formSearch.yiyuandizhi != '') searchWhere.yiyuandizhi = '%' + this.formSearch.yiyuandizhi + '%';
			let user = JSON.parse(localStorage.getItem('sessionForm'))
		if (this.sortType) searchWhere.sort = this.sortType
		if (this.sortOrder) searchWhere.order = this.sortOrder
        this.$http.get(`chongwuyiliao/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        ...
        this.$router.push({path: '/index/chongwuyiliaoDetail', query: params});
      },
