...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"0 0 0px","margin":"0px 0 0","borderColor":"#ddd","alignItems":"center","display":"flex","clear":"both","flexWrap":"wrap","background":"#fff","borderWidth":"0 0 0px","width":"100%","borderStyle":"solid","height":"auto","order":"1"}'>
      <el-form-item :style='{"margin":"0 10px 0 0"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 0 0 10px","lineHeight":"42px","textAlign":"right","display":"inline-block"}'>饲料名称：</div>
        <el-input v-model="formSearch.siliaomingcheng" placeholder="饲料名称" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"0 10px 0 0"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 0 0 10px","lineHeight":"42px","textAlign":"right","display":"inline-block"}'>饲料品牌：</div>
        <el-input v-model="formSearch.siliaopinpai" placeholder="饲料品牌" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
	  ...
	  <el-button v-if="btnAuth('chongwusiliao','新增')" :style='{"cursor":"pointer","border":"0px solid #ddd","padding":"0px 10px","margin":"0px 10px 0 0","color":"#fff","borderRadius":"0px","background":"rgba(30,124,9,1)","display":"inline-block","width":"80px","fontSize":"14px","lineHeight":"36px","height":"36px"}' type="primary" @click="add('/index/chongwusiliaoAdd')"><i v-if="true" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
		...
		<div class="list3 index-pv1" :style='{"border":"1px solid rgb(123, 123, 123)","padding":"10px","borderRadius":"10px","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","height":"auto"}'>
		  <div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
		    <div :style='{"border":"0px solid #f6f6f6","width":"100%","padding":"0px","margin":"0 0px 0 0","overflow":"hidden","height":"220px"}' class="img-box">
				<img @click.stop="imgPreView(item.siliaotupian)" :style='{"width":"100%","objectFit":"cover","display":"block","height":"100%"}' v-if="item.siliaotupian && item.siliaotupian.substr(0,4)=='http'" :src="item.siliaotupian.split(',')[0]" class="image" />
				<img @click.stop="imgPreView(baseUrl + (item.siliaotupian?item.siliaotupian.split(',')[0]:''))" :style='{"width":"100%","objectFit":"cover","display":"block","height":"100%"}' v-else :src="baseUrl + (item.siliaotupian?item.siliaotupian.split(',')[0]:'')" class="image" />
			</div>
		    <div :style='{"width":"100%","padding":"0 0px","overflow":"hidden","alignItems":"center","display":"flex","height":"auto"}' class="item-info">
		      <div :style='{"width":"100%","padding":"0 0px 0 0","textAlign":"center","flex":"1"}'>
				<div class="title">{{item.siliaomingcheng}}</div>
				<div class="title">{{item.siliaozhonglei}}</div>
				<div class="title">{{item.siliaopinpai}}</div>
		      ...
		      <div class="desc" v-html="item.siliaopeiliao"></div>
		    </div>
		  </div>
		</div>
	</div>
...
<script>
  export default {
    ...
    methods: {
      ...
      getFenlei() {
		this.$http.get('siliaozhonglei/list',{}).then(res => {
		  if (res.data.code == 0) {
		    this.fenlei = res.data.data.list
		  }
		});
		this.feileiColumn = 'siliaozhonglei'
      },
      getList(page, fenlei, ref = '') {
        ...
        let searchWhere = {};
        if (this.formSearch.siliaomingcheng != '') searchWhere.siliaomingcheng = '%' + this.formSearch.siliaomingcheng + '%';
        if (this.formSearch.siliaopinpai != '') searchWhere.siliaopinpai = '%' + this.formSearch.siliaopinpai + '%';
        if (this.curFenlei != '全部') searchWhere.siliaozhonglei = this.curFenlei;
			let user = JSON.parse(localStorage.getItem('sessionForm'))
		if (this.sortType) searchWhere.sort = this.sortType
		if (this.sortOrder) searchWhere.order = this.sortOrder
        this.$http.get(`chongwusiliao/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        ...
        this.$router.push({path: '/index/chongwusiliaoDetail', query: params});
      },
