		...
		<div class="attr" :style='{"padding":"20px 0","margin":"0","flexWrap":"wrap","background":"none","display":"flex","width":"100%","position":"relative","justifyContent":"space-between","order":"2"}'>
			...
			<div class="info" :style='{"width":"100%","padding":"0 0px","margin":"0","background":"#fff","order":"2"}'>
				...
				<div class="item" :style='{"padding":"0 0px","margin":"0 0 4px 0","borderColor":"#f3f3f3","background":"#fff","borderWidth":"0 0 0px","display":"flex","borderStyle":"solid","justifyContent":"spaceBetween"}'>
					<div class="lable" :style='{"padding":"0 10px 0 0","color":"#333","textAlign":"right","width":"150px","fontSize":"14px","lineHeight":"24px","fontWeight":"500","height":"auto"}'>宠物种类</div>
					<div  :style='{"padding":"0px 10px 0","fontSize":"14px","lineHeight":"24px","color":"#555","flex":"1","height":"auto"}'>{{detail.chongwuzhonglei}}</div>
				</div>
				<div class="btn" :style='{"padding":"10px 0px","flexWrap":"wrap","display":"flex"}'>
					...
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 10px","margin":"0 5px 0 0","outline":"none","color":"#333","borderRadius":"2px","background":"#f5340030","width":"auto","lineHeight":"36px","fontSize":"14px","height":"36px"}' v-if="btnAuth('chongwuzhonglei','私聊')&&detail.id!=mid" @click="chatClick">联系TA</el-button>
				</div>
			</div>
		</div>
  ...
  export default {
    ...
    methods: {
      ...
      nextClick(page) {
		...
		// 删除
		async delClick(){
			await this.$confirm('是否删除此宠物种类？')
			  .then(_ => {
			    this.$http.post('chongwuzhonglei/delete', [this.detail.id]).then(async res => {
					if (res.data.code == 0) {
						this.$message({
							type: 'success',
							message: '删除成功!',
