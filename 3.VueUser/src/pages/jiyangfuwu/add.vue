...
<div :style='{"width":"100%","padding":"30px calc((100% - 1200px)/2) 40px","margin":"0px auto","position":"relative","background":"#fff"}'>
    ...
    >
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="服务价格/天" prop="fuwujiage">
            <el-input v-model.number="ruleForm.fuwujiage" 
                placeholder="服务价格/天" clearable :disabled=" false  ||ro.fuwujiage"></el-input>
          </el-form-item>
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="服务简介" prop="fuwujianjie">
            <el-input
              type="textarea"
              :rows="8"
              placeholder="服务简介"
...
<script>
  export default {
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`jiyangfuwu/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('jiyangfuwu/list', {
							...
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`jiyangfuwu/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
